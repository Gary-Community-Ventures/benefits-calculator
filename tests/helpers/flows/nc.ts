  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (!setupResult.success) {
        console.error(`[NC-E2E] Setup failed at step: ${setupResult.step} (${setupResult.error?.message || 'Unknown error'})`);
      ...
      if (!applicationResult.success) {
        console.error(`[NC-E2E] Application flow failed at step: ${applicationResult.step} (${applicationResult.error?.message || 'Unknown error'})`);
